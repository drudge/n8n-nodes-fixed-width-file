		...
		if (operation === 'fromFile') {
			for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
				try {
					...
					const fieldManifest = this.getNodeParameter('fields', itemIndex, {}) as IDataObject;
					const fieldSetup = ((fieldManifest as IDataObject) || {}).field as IDataObject[]|| [];
					...
					const fields: Field[] = fieldSetup.map(({ property, width, align, dataType }: IDataObject) => {
						let cast;
					...
					// Read the binary fixed width file data
			...
			const fieldManifest = this.getNodeParameter('fields', 0, {}) as IDataObject;
			const fieldSetup = ((fieldManifest as IDataObject) || {}).field as IDataObject[]|| [];
			const fileName = options.fileName as string;
			const encoding = options.encoding as BufferEncoding || 'utf8';

			const fields: Field[] = fieldSetup.map(({ property, width, align }: IDataObject) => {
				return {
					property: property as string,
					width: width as number,
					align: (align as string) as "right" | "left" | undefined,
				};
			});
			...
			// console.log(stringifyOptions);
			const text = stringify(items.map(item => item.json), stringifyOptions);
			// Read the binary fixed width file data
			const binaryData = await this.helpers.prepareBinaryData(Buffer.from(text, encoding), fileName);
			items = [
				{
					json: {},
					binary: {
